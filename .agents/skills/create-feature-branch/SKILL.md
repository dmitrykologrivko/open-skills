---
name: open-skills:create-feature-branch
...
Use this skill to start work on a feature from a task reference.
...
2. Check out `{{base_feature_branch}}` and run `git pull` to get its latest changes. If the branch is missing or either command fails, stop and report the error. Do not force-pull or reset the branch.
...
6. If the sanitized branch name is empty or already exists, stop and report the problem. Otherwise create it from the updated base branch with `git checkout -b <branch name> {{base_feature_branch}}`.
